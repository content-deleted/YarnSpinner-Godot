...
namespace Yarn.GodotYarn {
    ...
    public class Actions : ICommandDispatcher {
        ...
        internal class CommandRegistration : ICommand {
            ...
            internal CommandDispatchResult Invoke(DialogueRunner dispatcher, List<string> parameters, out Delegate commandCoroutine, out object[] finalParametersToUse) {
                object target;
                ...
                if (DynamicallyFindsTarget) {
                    ...
                    var gameObject = NodeFindUtility.Find(gameObjectName);
                    ...
                    var targetComponent = System.Convert.ChangeType(gameObject, this.DeclaringType);
                ...
                if (this.TryParseArgs(parameters.ToArray(), out var finalParameters, out var errorMessage) == false) {
                    commandCoroutine = default;
                    finalParametersToUse = finalParameters;
        ...
        CommandDispatchResult ICommandDispatcher.DispatchCommand(string command, out Delegate commandCoroutine, out object[] finalParametersToUse) {
            ...
            if (_commands.TryGetValue(commandPieces[0], out var registration)) {
                ...
                return registration.Invoke(DialogueRunner, commandPieces, out commandCoroutine, out finalParametersToUse);
            } else {
                commandCoroutine = default;
                finalParametersToUse = default;
        ...
        public static void AddRegistrationMethod(Action<IActionRegistration> registerActions) {
            GD.Print($"Adding registration method {registerActions.Method.DeclaringType.FullName}.{registerActions.Method.Name}");
